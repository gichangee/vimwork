...
* #### 해설
  ...
  C-@(ctrl+@)는 앞에 입력모드에서  입력한 것을 그대로 입력한 후 일반모드로 다시 돌아간다. (즉 " 입력후 ESC 키를 누른것과 동일하다.)
...
* #### 해설
  ...
  *는 커서 뒤에 있는 단어를 선택한다. (특수문자 무시)
 
  :s/(원래 문자열)/(변경 문자열)/g는 현재 행에 있는 모든 (원래 문자열)을 (변경 문자열)로 바꾼다. 
 
  g는 한 행에 (원래 문자열)에 입력한 패턴이 여러번 나오면 그 행 전체를 변경한다는 의미

  위에서는 이미 *로 단어를 선택해서 // 사이를 공백으로 씀
 
  CR은 (Enter)
   
  )는 다음 문장의 처음으로 커서를 이동한다.
   
  대문자 B는 문자 단위로 커서를 뒤로 이동한다. (특수문자 신경x)
   
  *는 커서 오른쪽에 있는 단어를 선택한다. (특수문자 무시)
...
* #### 해설
   ...
   -는 위의 행으로 이동하고 그 행의 맨 앞 문자 앞으로 커서를 이동한다.
   ...
   @q 지금까지 기록한 것을 1번 반복한다. (즉, -O// < C-N > TODO < ESC > 을 현재 커서에서 실행시킨다.)
...
* #### 해설
  ...
  f1은 지금 커서에서 오른쪽으로 갔을 때 처음 만나는 1앞으로 커서를 옮긴다. 그 행에서 1이 없다면 다음 행까지 찾지는 않는다.
 
  qq(기록하고 싶은 것)q 는 qq를 친 다음 모든 것들을 기록 해두고 마지막에 q를 쓰면 그 기록이 저장 된다
 
  r4는 지금 커서 오른쪽에 있는 것을 4로 교체한다
 
  UP은 커서를 한칸 위로 올린다
  
  r3는 지금 커서 오른쪽에 있는 것을 3로 교체한다
 
  UP은 커서를 한칸 위로 올린다
 
  r2는 지금 커서 오른쪽에 있는 것을 2로 교체한다
 
  f1은 지금 커서에서 오른쪽으로 갔을 때 처음 만나는 1 앞으로 커서를 옮긴다. 그 행에서 1이 없다면 다음 행까지 찾지는 않는다.
  ...
  23h 커서를 23칸 왼쪽으로 옮긴다.
 
  rb 커서 오른쪽에 있는 것을 b로 교체한다.
 
  DOWN은 커서를 한칸 내린다
  
  rr 커서 오른쪽에 있는 것을 r로 교체한다.
 
  DOWN은 커서를 한칸 내린다
 
  rg 커서 오른쪽에 있는 것을 g로 교체한다. 
...
* #### 나의 점수 : 29 점
* #### 나의 정답
  * 4jwyw5jf"pa,name,age,score< ESC >ZZ
* #### 해설
 
  4j는 커서를 4칸 내린다
 
  소문자 w는 다음 단어의 앞으로 이동한다.(특수문자도 하나의 단어로 침 ABC: A앞에 커서가 있고 w를 누르면 : 왼쪽으로 커서가 이동)
 
  yw는 현재 word의 끝까지 복사함
 
  5j는 커서를 5칸 내린다
 
  f" 지금 커서에서 오른쪽으로 갔을 때 처음 만나는 "앞으로 커서를 옮긴다. 그 행에서 "이 없다면 다음 행까지 찾지는 않는다.
 
  소문자 p는 현재 커서 한칸 뒤에 붙여넣기
 
  소문자 a는 현재커서 오른쪽 한칸 옆에서 입력모드를 시작한다.
 
  ,name,age,score 는 그냥 입력모드에서 입력한 것
  
  ESC는 입력모드에서 일반모드로 변경
